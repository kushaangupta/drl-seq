...
class TrialEnv(BaseEnv):
    ...
    def post_step(self, ob, reward, done, truncated, info):
        """
    ...
    def new_trial(self, **kwargs):
        ...
        self._ob_built = False
        self._gt_built = False
        trial, info = self._new_trial(**kwargs)
        self.trial = trial
    ...
    def step(self, action):
        ...
        ob, reward, done, truncated, info = self._step(action)
        ...
        if info['new_trial']:
            ...
            trial, info_ = self._top.new_trial()
            info.update(info_)
            self.performance = 0
            info['trial'] = trial
        if ob is OBNOW:
            ob = self.ob[self.t_ind]
        return self.post_step(ob, reward, done, truncated, info)

    def reset(self, seed=None, return_info=True, options=None, step_fn=None, no_step=False):
        ...
        if step_fn is None:
            ob, _, _, _, info = self._top.step(self.action_space.sample())
        else:
            ob, _, _, _, info = step_fn(self.action_space.sample())
        return (ob, info) if return_info else ob
