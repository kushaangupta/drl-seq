...
def run_env(env, num_steps=200, num_trials=None, def_act=None, model=None):
    ...
    gt = []
    perf = []
    ob, info = env.reset()  # TODO: not saving this first observation
    ob_cum_temp = ob
    ...
    for stp in range(int(num_steps)):
        ...
        ob, rew, done, _, info = env.step(action)
        ob_cum_temp += ob
        ...
        if info['new_trial'] or 'trial' in info.keys():  # TODO: fix hack
    ...
    data = {
        'ob': np.array(observations).astype(np.float64),
        'ob_cum': np.array(ob_cum).astype(np.float64),
        'rewards': rewards,
        'actions': actions,
        'perf': perf,
...
def plot_env_1dbox(
    ...
    if ob_traces:
        ...
        ax.imshow(ob.T, interpolation='none', aspect='auto', origin='lower')
    ...
    if performance is not None:
        ...
        mean_perf = np.nanmean(performance[performance != -1])
    ...
    if states is not None:
        ...
        ax = axes[i_ax]
        i_ax += 1
        plt.imshow(states[:, int(states.shape[1]/2):].T, interpolation='none',
                   aspect='auto')
...
def plot_env_3dbox(ob, actions=None, fname='', env=None):
    ...
    ax.axis('off')
    im = ax.imshow(ob[0], interpolation='none', animated=True)
